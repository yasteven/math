% REORGANIZATION NOTE: This version restructures the restored draft into a cleaner derivation pipeline and corrects the ball ontology: Info Balls are photon/macrophoton injection carriers, while quasars are treated as post-Info-Ball-injection anti-core/anti-proton-like candidates, not Info Balls directly.
% =====================================================================
% THE FACT OF FRACTAL TENNIS (TFOFT) — MERGED FINAL DRAFT
% Original + All requested additions — May 2026
% =====================================================================
...
\label{sec:alpha_deriv}
  ...
  \begin{quote}
    ...
    The operational rules of  self-describing Presburger-ASSP machine minimize information cost subject to MATHICCS constraints. Since the physical registers are literal spheres, the radius, circumference, surface, rotation, and displacement channels must be encoded using the same reusable geometric word whenever possible. The Presburger-minimal basis radius is therefore forced to be \(r=\pi\): choosing \(r=1\) introduces an additional lookup between the register's geometric word and its rotational/surface channels, while \(r=\pi\) collapses all radius references into a single persistent symbol. This is not a numerical fit but  minimal-code consequence of Axiom P2 under the ASSP substrate
  \end{quote}
...
\begin{enumerate}
  ...
  \item The angular-and-linear momentum wave in the higher fractal layer \emph{is} the info ball.
  \item The computational cost is paid by injecting dust balls into the lower fractal layer.
\end{enumerate}
...
When two balls form a gyro ball, the system promotes a 1D Presburger update into a 3D Euclidean tangency update. Using a Strassen-inspired fast-multiplication analogy~\cite{Strassen1969,Laderman1976,Heule2019}:
...
The natural logarithm of the electron-to-solar-mass ratio is $\ln(M_\odot/m_e) \approx 138.9357$~\cite{CODATA2022}.


\subsection{The Ball Ontology: Fundamental Objects of the FTC}

\paragraph{Info-Ball clarification.}
An Info Ball is identified with the photon/macrophoton only at the injection event. During transport it is not directly observed as a photon at any scale; after injection the observable object is a reconstructed post-injection state. Therefore a quasar is not classified as an Info Ball in this ontology. It is treated as a post-Info-Ball-injection anti-core candidate, most naturally macro-antiproton-like, while a stellar-mass black hole is the macro-scale analogue of an anti-electron.
...
\hline
Info Ball  & Photon/macrophoton injection carrier: angular-momentum information from scale $A$ carried into scale $A{+}1$ as a mostly-linear-momentum Kick Ball. After injection it is no longer directly observable as a photon at any scale; transport is inferred only from post-injection states. & Photon at injection & Macrophoton at injection \\
\hline
...
\hline
Fast Ball  & Higher-power coherent injection from scale $A{+}1$ out of the $A$ core-ball poles; astrophysically observed through post-injection anti-core/anti-proton-like states rather than direct photon transport. & Gamma-ray injection & Quasar / anti-core candidate \\
\hline
...
We conjecture that the positron is a star ball whose electromagnetic handedness has been inverted by a Möbius transformation $r \to R^2/r$:
\begin{align*}
m_{\rm anti} &= m_{\rm star}, \quad s_{\rm anti} = -s_{\rm star}, \quad q_{\rm anti} = -q_{\rm star}.
\end{align*}


\subsection{Neutrino Flavor Channels as Sub-Stellar Lifecycle Stages}

The three neutrino channels correspond naturally to the three passive sub-stellar lifecycle stages:
\[
\nu_e \leftrightarrow \mathrm{rocky/terrestrial\ channel},\quad
\nu_\mu \leftrightarrow \mathrm{gas\ giant\ channel},\quad
\nu_\tau \leftrightarrow \mathrm{brown\ dwarf\ channel}.
\]
No fourth light neutrino is expected because no fourth Soddy-stable passive sub-stellar regime exists between the brown-dwarf channel and the active Star Ball regime~\cite{NuFIT2024}.
...
The binding energy stabilized by completing one round-trip is
\begin{equation}
...
The first physical object is not a single sphere. It is a two-pole
star--core object:
\begin{equation}
...
\end{equation}
The Yb-174 prediction is
\begin{equation}
\boxed{
\alpha_{Yb174}^{-1} \approx 137.035998952
}
\end{equation}
which is the primary near-term recoil test of the model.
...
\section{Gravity from Soddy Geometry and the Derivation of \(G\)}
...
\section{Dark Matter as Lower-Scale Fractal Substrate}
...
Mature, metal-rich, structured quasar host galaxies at high z (JWST) & Quasars are recycled dense galaxy cores ejected from parent macro-stars. They carry pre-existing mixed stellar populations + enriched material from the parent. The quasar phase is a compact relaxation stage that rapidly reorganizes into normal galactic structure using recycled fusion products and non-local jet inflow. JWST “early” galaxies are naturally explained as recently ejected material still relaxing. \\

Continuous Lyman-alpha forest with cosmological clustering statistics & The forest is primarily a photonic register readout of the quasar’s core-crossing birth event + its radial ejection trajectory through the fractal substrate. Secondary absorption from intervening ejected macro-H (lower-layer hydrogen) modulates the signal according to local fractal density. Uniform base pattern expected from identical hydrogen-like registers. \\

Time dilation in quasar variability and supernovae & Differential clock rates in the macro-star gravitational/fractal potential well. Quasars preserve their birth-clock redshift from manufacturing depth. \\

Gravitational lensing time delays & Geometric delays + differential ejection depth / clock desynchronization between lensed images. \\

Lack of statistical parent-galaxy associations in large surveys & Many quasars originate from naked cores (quiet massive black holes). Visible Arp-style associations occur when the parent core has a stellar flock. \\

Mass budget for building massive hosts & Quasars are dense recycled cores carrying substantial baryons from the parent macro-star plus non-local material inflow during crossing. No primordial gas required. \\

Large-scale isotropy & Radial isotropic ejection from macro-star + fractal self-similarity appears homogeneous at our scale. Residual dipoles (CMB and quasar) remain. \\

Quasar dipole ~94° offset from CMB dipole & Quasars have 3--5× faster ejection velocity due to higher manufacturing energy. The offset arises from angular momentum precession during parallel macro-gamma-ray core-crossing injection. \\

Continuous quasar redshifts + Karlsson periodicity & Macro-gamma-ray production across continuous depths + outward motion of target macro-H smears birth locations (continuous distribution). Discrete core-crossing events produce the quantized Karlsson steps. \\

\end{longtable}


% ==================================================================
%  RESTORED FROM AI.VIXRA DRAFT: OBSERVATIONAL EVIDENCE AND DERIVED CONSTANTS
% ==================================================================
...
Voltage is the pressure gradient driving the recoalescence process.
...
A superconductor is a lattice phase in which electron-stars remain
coalesced across the channel. Zero resistance does not mean no dynamics;
it means no repeated destruction/reassembly cycle.
...
The apparent objection that stars do not repel like electrons is based on
testing the analogy at the wrong scale and time. The effective interaction is delayed by the fractal layer:
\begin{equation}
F_{\star\star}(d,t)
=
F_{\rm bind}(d)
...
\Theta(t-d/v_{\rm eff}),
\end{equation}
where \(F_{\rm bind}\) is the galactic/core binding contribution and
\(F_{\rm source}\) is the delayed source-source repulsion. The mature electron-like repulsion appears only after sufficient distance and fractal-layer time have allowed the source shells to form:
\begin{equation}
\mathrm{electron} \leftrightarrow \mathrm{star},\quad
\mathrm{electron\ repulsion} \leftrightarrow \mathrm{mature\ stellar\ source\ repulsion}.
\end{equation}
...
\section{Quasars, Post-Info-Ball Injection Objects, and Global Shuffling Signals}
...
The quasar is therefore not merely an active galactic nucleus. It is a
macro-antiproton phase that eventually relaxes, decays, and manufactures normal galactic structure.
...
In the full macro-star ejection ontology, the visible universe consists of macro-hydrogen (galaxies) ejected radially outward from a parent wish-scale macro-star undergoing fusion. Gamma rays produced continuously across a range of depths in the macro-star’s fusion zone trigger quantized core-crossing events in the ejected macro-H atoms, manufacturing quasars (post-macrophoton-injection anti-core candidates; macro-antiproton-like objects). These quasars preserve the birth-clock redshift of the exact fractal layer / potential depth where they were manufactured.
...
\subsection{Quasars as Post-Info-Ball Injection Objects and the Karlsson Logarithmic Periodicity}

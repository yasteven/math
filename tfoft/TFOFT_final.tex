...
needed to define that state relationally.
...
\[
T_{\rm sphere}^{\rm TFOFT} = 4\pi^3+\pi^2+\pi = 137.0363038.
\]
The rank-23 two-ball geometry supplies the raw dual-channel leakage:
\[
...
= 0.2150079.
\]
The complementary mass step is:
\[
k_{\rm mass}^{\rm TFOFT} = 2-k_{\rm info}^{(0),{\rm TFOFT}} = 1.7849921.
\]

\subsubsection{Why 77 Layers: The Half-Boundary Mass Register}
\label{sec:why_77_layers}

The Star Ball mass register receives only half the rank-23 curvature boundary:
\[
T_{\rm mass}^{\rm TFOFT}
=
...
Newton's form of Kepler's third law~\cite{Newton1687}:
\[
T^2=\frac{4\pi^2}{GM}a^3
\]
can be written in ASSP form as
\[
\boxed{
...
\]
The factor $4\pi^3$ is the universal Soddy surface register cost already appearing in the single-sphere state budget, $T_S=4\pi^3$.

\subsection{Rydberg Scaling from Virtual-Core Clock Quantization}
\label{sec:rydberg_virtual_core}

The allowed boundary energies are
\[
\boxed{
E_n = -\frac{E_{\rm atom}}{n^2}.
}
\]
For hydrogen, $E_n=-13.606\,{\rm eV}/n^2$. A photon is emitted when the virtual Core Ball relocks:
\[
E_\gamma = E_{\rm atom}\left(\frac{1}{n_f^2}-\frac{1}{n_i^2}\right).
\]

\subsection{Nuclear Binding Energy: The Soddy Fractal Nucleon Stack}

The minimal 3D Soddy-balanced configuration is four mutually tangent equal spheres arranged in a regular tetrahedron---consistent with $^4$He having the highest binding energy per nucleon among light nuclei. Iron ($^{56}$Fe) is conjectured to sit at the global binding energy maximum because it is the first Soddy iteration at which both inner and outer Soddy cores are simultaneously real and balanced.
...
\hline
Star Ball  & Fundamental information register; stores and transfers angular momentum via rotation. & Electron & Star \\
\hline
Core Ball  & Boundary port; converts incoming angular momentum into outgoing linear-momentum info balls. & Proton / Nucleus & Galactic core \\
\hline
Gyro Ball  & Star Ball orbiting Core Ball; the complete fractal circuit. & Hydrogen atom & Spiral galaxy \\
\hline
Info Ball  & Angular-momentum information from scale $A$ carried in scale $A{+}1$. & Photon & Macrophoton \\
\hline
Dust Ball  & Linear momentum computational cost exhausted to scale $A{-}1$. & Electron cloud & Dark matter halos \\
\hline
Pipe Ball  & Elongated Soddy tangency chain oriented along angular momentum gradient. & Gluon field & Cosmic web filament \\
\hline
\end{tabular}
\caption{TFOFT Ball Ontology (abbreviated). Cross-scale correspondences are proposed exact fractal-scale equivalences.}
\end{table}
...
The Laurent expansion of the Möbius map gives an effective gravitational acceleration:
\begin{equation}
...
\label{eq:laurent}
\end{equation}
where $r_s = 2GM/c^2$. The zeroth-order term is consistent with flat galactic rotation curves directly from the Möbius geometry, without requiring additional dark matter at large radius.
...
In the full macro-star ejection ontology, the visible universe consists of macro-hydrogen (galaxies) ejected radially outward from a parent wish-scale macro-star undergoing fusion. Gamma rays produced continuously across a range of depths in the macro-star’s fusion zone trigger quantized core-crossing events in the ejected macro-H atoms, manufacturing quasars (macrophotons / macro anti-protons). These quasars preserve the birth-clock redshift of the exact fractal layer / potential depth where they were manufactured.
...
Lack of statistical parent-galaxy associations in large surveys & Many quasars originate from naked cores (quiet massive black holes). Visible Arp-style associations occur when the parent core is active. \\
...
Quasar dipole ~94° offset from CMB dipole & Quasars have 3--5× faster ejection velocity due to higher manufacturing energy. The offset arises from angular momentum precession during parallel gamma-ray core-crossing injection. \\

Continuous quasar redshifts + Karlsson periodicity & Gamma-ray production across continuous depths + outward motion of target macro-H smears birth locations (continuous distribution). Discrete core-crossing events produce the quantized Karlsson steps. \\

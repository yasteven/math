...
% Add this new subsection after \section{Logical Inconsistency} and before FSEP section

\subsection{Formalization in First-Order Logic with Arithmetical Encoding}

Let $\mathcal{L}_{\text{GR}}$ be first-order logic with signature:
\begin{itemize}
\item Manifolds $M$, Lorentzian metrics $g_{\mu\nu}$, curvature tensors $R^\rho_{\sigma\mu\nu}$, EFE predicate $\text{EFE}(g,T) \equiv G_{\mu\nu} + \Lambda g_{\mu\nu} = \frac{8\pi G}{c^4} T_{\mu\nu}$
\item Constants: $\varepsilon > 0$, length scales $\ell > 0$
\end{itemize}

\textbf{Axioms of GR:}
\begin{enumerate}
\item \textbf{EEP Schema:} $\forall p \in M \ \forall \varepsilon > 0 \ \exists U_p \ \exists \ell(\varepsilon) \ \big( |R^\rho_{\sigma\mu\nu}| \ell^2 < \varepsilon \ \land \ |g_{\mu\nu} - \eta_{\mu\nu}| < \varepsilon \ell^2 \ \land \ \text{local Minkowski} \big)$
\item Smoothness and causality axioms on $(M,g)$
\item EFE schema: $\text{EFE}(g,T)$ for matter $T$
\item Continuum limit existence (compact convergence $C^k$)
\end{enumerate}

\textbf{Theorem [Syntactic Inconsistency]:} $\mathcal{L}_{\text{GR}} \vdash \exists g \big( \text{EFE}(g,0) \land \forall \varepsilon,\ell \ \neg \text{EEP}(\varepsilon,\ell,g) \big)$

\textbf{Proof:} Construct $S_\infty = \lim_{N\to\infty} \text{CFM}_N$ via inductive Apollonian hierarchy (compact convergence in $C^k$ topology by $\sum m_n < \infty$, Hausdorff dim $\approx 2.4739 < 3$). Each finite $\text{CFM}_N \vDash \text{EFE}$ (weak-field superposition + selection). Limit inherits EFE in vacuum by standard PDE convergence. But $\forall \varepsilon,\ell > 0$, sub-hierarchy at iteration $N(\varepsilon,\ell)$ yields $|R| \ell^2 \geq \delta(\varepsilon) > 0$ by diagonal construction.

Thus: $\text{EEP} \land \text{EFE} \land S_\infty \vdash \text{EEP} \land \neg \text{EEP}$.

\textbf{Explosion:} $\mathcal{L}_{\text{GR}} \vdash \bot \implies \mathcal{L}_{\text{GR}} \vdash \forall \phi \ (\phi)$

In particular, via arithmetical embedding (G\"odel numbering of $\mathcal{L}_{\text{GR}}$ formulas into causal structure, cf. Malament-Hogarth spacetimes~\cite{Hogarth1992}), $\mathcal{L}_{\text{GR}} \vdash ``1=0''$.

This is stronger than semantic inconsistency: GR's first-order axiomatization is \emph{syntactically trivial}.
...
\bibitem{Hogarth1992}
Hogarth, M. Does General Relativity allow an observer to prove the completeness theorem? {\em Class. Quantum Grav.} \textbf{9}, L69--L74 (1992).


\bibitem{Elliott2026FSEP}
Elliott, S.~E. The fractal substrate equivalence principle: A unified foundation for quantum mechanics and general relativity. viXra:2601.0119 (2026). Available at: \url{https://vixra.org/abs/2601.0119}.

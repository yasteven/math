...
\begin{abstract}
The companion paper establishes that the universe is a finite computational
object defined by a dynamical Apollonian--Soddy sphere packing fractal quine
under Presburger arithmetic. This paper develops the continuum limit of that
framework. The sphere radius acts as a local clock field whose coarse-grained
dynamics produces Newtonian gravity and the weak-field time component of
general relativity. On the same tangency graph, the fundamental quantum channel
is shown to be effectively one-dimensional: an Info Ball propagates along an
active tangency line while a two-state boundary-spin register records the
clockwise/anti-clockwise, or left/right, half-turn state. The continuum limit of
this chiral tangency walk is the \(1+1\)-dimensional Dirac equation, with mass
appearing as the local chirality-reversal rate set by the radius-clock tick.
The surrounding three-dimensional packing supplies the embedding geometry,
radial clock field, and local frame rotations, but the controlled Dirac
derivation itself requires only the active one-dimensional transfer channel.
The extension to full curved \(3+1\)-dimensional spinor dynamics is therefore
stated as a conjectural coarse-grained frame-rotation ansatz rather than as a
proved theorem. A visual-relocation interpretation through a spherical
tangency lens yields a conjectural route to the full Einstein tensor without
literal spacetime curvature. The model predicts an equatorial Fermi donut in
the Milky Way at 10 to 15 kiloparsecs with one-quarter the angular density
contrast of the polar Fermi bubbles, placing the key observational test in the
outer-disk region where Gaia DR3 data already show model-sensitive rotation
curves.
\end{abstract}
\tableofcontents
\newpage

...
proved theorem. The paper does not assume the Einstein Equivalence Principle as fundamental:
mass is treated as an angular-momentum register, while inertia is the linear
momentum cost required to translate that angular-momentum system through the
radius-clock substrate. The full Einstein tensor is therefore not derived here;
it is deferred to a later tensor-sector program. The model predicts an
equatorial Fermi donut in the Milky Way at 10 to 15 kiloparsecs with
one-quarter the angular density contrast of the polar Fermi bubbles, placing
the key observational test in the outer-disk region where Gaia DR3 data already
show model-sensitive rotation curves.
\end{abstract}
\tableofcontents
\newpage

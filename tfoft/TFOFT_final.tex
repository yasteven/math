...
\begin{abstract}
We propose a new principle of physics called MATHICCS, requiring that every mathematical axiom used in a physical law must correspond to an operation persistently realizable by a physical subsystem. Applying this criterion leads to a discrete, self-similar, singularity-free geometry: the Apollonian-Soddy Sphere Packing (ASSP). We conjecture that three-dimensional dynamics on this packing forms a self-computing fractal quine whose registers store angular and linear momentum as cross-scale equivalents. This framework derives the fine-structure constant as alpha inverse equals 4 pi cubed plus pi squared plus pi, or 137.036, from the minimal Presburger computational cost of a Soddy sphere state update. It derives the 77-layer star-electron hierarchy from the rank-23 half-boundary mass register, the hydrogen binding energy from the Gyro Ball equation, and the gravitational constant G from atomic structure by two independent first-principles routes. Empirical cross-checks include the 25-arcminute CMB seam, the observed electron-star hierarchy, and the near-match with Karlsson quasar logarithmic periodicity. A pending falsifiable prediction is given for the ytterbium fine-structure constant. The model is presented as a zero-free-parameter ontology whose derived consequences are compared against major tensions in observational physics.
\end{abstract}
...
\]
implies that any finite-energy localized configuration radiates thermally.
The decay time of a clock of energy $E$ is finite:
\[
...
For any finite energy $E > 0$, define $T_{\rm construct}(E)$ as the cosmic time after which it becomes impossible to construct---not merely maintain---a clock of energy $E$ at an arbitrary point in the expanding spacetime. Because the de~Sitter decay rate is nonzero and the accessible comoving volume available for apparatus construction shrinks due to the cosmological horizon,
$T_{\rm construct}(E)$ is finite for every $E > 0$:
\[
\exists\; T_{\rm construct}(E) < \infty \quad \forall\; E > 0.
\] 
Hence no clock of energy $E$ can be constructed everywhere in the universe after cosmic time $T_{\rm construct}(E)$. The set of spacetime regions in which a valid $\varepsilon$-$\delta$ measurement apparatus can be
\emph{constructed} therefore shrinks to measure zero:
\[
\mathcal{V}_{\rm constructible}(t) \to 0 \quad \text{as} \quad t \to \infty.
\]

\paragraph{Why the long timescale does not rescue EEP.}

A common objection notes that $\tau_{\rm decay}(E)$ for a macroscopic clock is astronomically large. This objection misidentifies what is at stake. The issue is not whether EEP fails \emph{today}, but whether it qualifies as a \emph{foundational axiom} at all. An axiom that is only constructively realizable for a finite duration $T_{\rm construct}(E) < \infty$ in a temporally unbounded universe has measure-zero validity over the full temporal domain. Foundational axioms cannot have expiration dates.
 
More precisely: the limiting procedure invoked by EEP---shrinking the apparatus to zero size while holding the measurement valid---itself requires that the apparatus can be constructed at each step of the limit. Once $t > T_{\rm construct}(E)$, the limit cannot be taken because the intermediate steps of the construction are physically unavailable. The logic of the limit therefore cannot be used to argue that EEP holds in the limit, since the limit procedure has already failed.

This can be stated as a clean theorem-like result:

\begin{quote}
\textbf{Theorem-like statement.} In any spacetime with $\Lambda > 0$, for every finite energy $E > 0$ there exists a finite cosmic time $T_{\rm construct}(E)$ after which the constructive limiting procedure required by EEP cannot be carried out at an arbitrary spacetime point. Therefore EEP is not a MATHICCS-valid foundational axiom: it is an approximation with a known and finite domain of constructive validity. General relativity with $\Lambda > 0$ is thus operationally self-limiting under MATHICCS---the same cosmological constant required to match observation also destroys the persistent constructibility required to define the theory's foundational local measurements.
\end{quote}
...
Presburger arithmetic lacks general variable multiplication; all operations reduce to repeated addition and finite symbol manipulation. In the Presburger-ASSP machine, every symbol, operation, and lookup constitutes a physical computational cost.

\paragraph{Numbers as programs; the word-size question.}

A critical clarification is required before the cost model can be stated. MATHICCS contains no abstract numbers. Every quantity in the Presburger-ASSP machine is a \emph{physical program} executed by the substrate of no-slip rotating spheres in a sphere packing. The symbol ``$\pi$'' is not a transcendental real number imported from external mathematics; it is the program that a sphere executes when it reports its own circumference-to-diameter ratio---a directly and persistently realizable physical operation on the substrate.

The analogy to digital computing is precise. On a digital circuit, \texttt{u32(1)} and \texttt{u8(1)} are not the same object even though both represent the concept ``one.'' The word size is determined by the physical register, not by abstract arithmetic. Asking whether $\pi$ is ``allowed'' in Presburger arithmetic commits the same error as asking whether a 32-bit register is ``allowed'' to hold a value that doesn't fit in 8 bits: it confuses the arithmetic with the hardware. 
In the Presburger-ASSP machine, the physical registers are literal spheres undergoing no-slip rotation in a sphere packing. The natural \emph{word size} of this substrate---the primitive quantity that every physical operation either reads or writes---is the sphere itself. The sphere's intrinsic dimensional unit is its radius, and the ratio of its circumference to its diameter is $\pi$. This is not chosen; it is measured from the register.
...
\boxed{r = \pi}.
\]
This is not a choice of a transcendental number over integers. It is the identification of the physical word size of the substrate, after which Presburger minimization selects the basis in the usual way.
\end{quote}
...
\begin{tabularx}{0.98\textwidth}{@{}p{2.5cm}YYY@{}}
\toprule
\textbf{Scale} &
\textbf{Luminous / addition-efficient channel} &
\textbf{Dark / overhead channel} &
\textbf{Interpretation} \\
\midrule

Atomic &
Discrete photonic transition channel &
Electron cloud as lower-scale halo analogue &
Visible transfer vs.\ support structure \\

Galactic &
Stars and gas &
Dark matter halo &
Luminous baryonic layer vs.\ stabilizing overhead \\

Cosmic &
Baryons and radiation &
Dark matter / dark-energy sector &
Observable matter vs.\ curvature-support substrate \\

\bottomrule
\end{tabularx}

\caption{Fractal congruence of visible and overhead channels. The
addition-efficient \(4/27\) channel appears as luminous transfer, while the
rank-23 overhead channel appears as lower-layer support structure.}
\label{tab:visible_dark_channels}
...
A gyro ball consists of one star ball orbiting one core ball. It is the minimal stable fractal register in the FTC and is conjectured to manifest as a hydrogen atom at the atomic scale and as a spiral galaxy at the astrophysical scale.

The electromagnetic channel passes exactly \(\alpha = 1/T_{\rm sphere}\) of the angular momentum per Presburger tick. A complete round-trip (star ball emits an info ball, core ball absorbs) requires two channel crossings. The binding energy stabilized by completing this round-trip is
...
The core ball's consumption rate greatly exceeds the star ball emission rate (\(m_{\rm core}c^2 \gg m_{\rm star}c^2\)), so the core is never the throughput bottleneck. The star ball alone sets the channel ceiling. This mass asymmetry establishes a directed computational flow (Star Ball \(\to\) Info Ball \(\to\) Core Ball), turning the gyro ball into a true communication channel rather than a symmetric dynamical system.

For hydrogen (\(m_{\rm star} = m_e\)) and the TFOFT value \(\alpha^{-1} = 4\pi^3 + \pi^2 + \pi \approx 137.036\), this recovers the standard Bohr/Rydberg ground-state binding energy to high precision:

\[
E_H = \frac{\alpha^2 m_e c^2}{2} \approx 13.606\ \text{eV}.
\]

For heavier atoms (\(Z > 1\)), increased nuclear charge compresses the effective unit radius \(\pi \to \pi(1 + \varepsilon(Z))\), yielding a local coupling

\[
\alpha(Z) = \frac{1}{T_{\rm sphere}\,(1+\varepsilon(Z))^3}, \qquad
E_{\rm atom}(Z) = \frac{\alpha(Z)^2 m_e c^2}{2}.
\]

\subsection{Derivation of the Electron Rest Mass from Recursive Soddy Curvature}

The electron rest mass \(m_e\) emerges from the recursive curvature dynamics of the Apollonian-Soddy sphere packing starting with the fundamental radius \(r = \pi\).

Numerical iteration of the Descartes--Soddy relation with branching (multiple new spheres per generation, starting from four mutually tangent spheres of radius \(\pi\)) shows that \(\ln k\) grows approximately linearly with generation depth. At generation 39 (roughly half the 77-layer mass register), the cumulative \(\ln k \approx 82.73\).

Applying the Strassen-inspired luminous channel fraction \(23/27 \approx 0.85185\) to this value yields

\[
82.73 \times \frac{23}{27} \approx 70.4,
\]

which aligns closely with half the two-ball quine depth (\(T_{\rm two-ball}/2 \approx 69.48\)).
...
matching the rank-23 Strassen curvature overhead. This confirms that core manufacturing corresponds to one full mass-register cycle modulated by the dominant Soddy/Strassen factor.

\textbf{Closure Condition.} The fundamental Star Ball mass \(m_e\) is the unique value such that the cumulative log-curvature growth over the 77-layer half-boundary mass register, scaled by the Strassen luminous fraction and combined with the Presburger register costs \(T_{\rm sphere} \approx 137.036\) and \(T_{\rm two-ball} \approx 138.955\), produces a self-consistent fractal quine. This fixes \(m_e\) so that

\[
\ln\left(\frac{M_\odot}{m_e}\right) \approx T_{\rm two-ball} \approx 138.955
\]

holds as an exact internal relation of the ASSP architecture.

Thus \(m_e\) is derived purely from the recursive Soddy curvature structure under Presburger cost minimization with initial radius \(r = \pi\). No external laboratory mass measurement is required as a primary input. All other constants of the theory (\(\alpha\), \(E_H\), \(G\), mass hierarchies, etc.) follow directly from this self-describing fractal quine.
...
\begin{tabularx}{0.98\textwidth}{@{}p{2.2cm}p{2.4cm}p{2.8cm}Y@{}}
\toprule
\textbf{Channel} &
\textbf{Mass ceiling} &
\textbf{Sub-stellar regime} &
\textbf{Interpretation} \\
\midrule

Electron flavor &
Rocky-planet scale &
Terrestrial / metallic bodies &
Lowest curvature-storage channel; stable condensed matter regime \\

Muon flavor &
Gas-giant scale &
Jovian-class bodies &
Intermediate overhead-storage channel with large volatile envelope \\

Tau flavor &
Brown-dwarf scale &
Failed-star transition regime &
Highest sub-stellar channel before sustained fusion ignition \\

\bottomrule
\end{tabularx}

\caption{FTC comparison between neutrino flavor-channel mass ceilings and
sub-stellar lifecycle stages. The ratios are interpreted as upper channel
ceilings rather than exact measured flavor masses. The three channels align
with the rocky, gas-giant, and brown-dwarf sub-stellar regimes.}
\label{tab:neutrino_substellar_channels}
...
This near-match motivates the identification of quasars as candidate
macroscopic Info Ball events: high-angular-momentum macrophoton-like transfer
events associated with parent macro-star/Core Ball coupling.

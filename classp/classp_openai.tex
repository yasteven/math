...
% Revised corrected draft with oriented 3D Soddy and Newton-channel fixes --- May 2026
% =====================================================================
...
descriptions of finite tangency updates. This revised paper develops a continuum-limit skeleton of the ASSP program, with the microscopic curvature law stated in its oriented three-dimensional sphere form, with the M\"obius Newton channel normalized by the final two-surface norm-map cost, and with a conjectural visual-relocation ansatz for the tensor sector. We define a tangency graph
...
near $R\sim0.8H_{\rm FB}\sim8$--$12\,\kpc$, with one-quarter the
polar-lobe angular density contrast, and with the relevant disk-halo boundary
searched radially outward from the Galactic center. This places the test in the same radial
...
argument alone does not derive the full Einstein tensor; the next subsection
states a conjectural visual-relocation ansatz for the tensor sector, while the
strong-field microscopic derivation remains a separate CLASSP task.

\subsection{Visual relocation ansatz for the Einstein tensor}
\label{sec:visual_relocation}

In this framework the Einstein tensor is interpreted as the continuum record of
apparent matter relocation through a spherical tangency lens, rather than as an
ontologically primitive smooth spacetime curvature.  The ASSP substrate is a
locally finite network of spheres and tangencies.  A bounded observer is a local
cluster of spheres, and distant matter is seen through shortest available
tangency paths.  The observer's local spherical packing therefore acts as a
lens: matter is not required to bend an underlying continuum; instead, the
observer's visual register assigns an apparent displaced position to matter
seen through the global tangency network.

Let $\xi^a(x)$ denote the apparent relocation vector of matter at a continuum
point $x$, measured by a local spherical observer.  A graph-level prototype is
\begin{equation}
  \xi_i^a=
  \frac{1}{k_{\rm local}}
  \sum_{j\in\Gamma_i} w_{ij}s_{ij}^a,
  \label{eq:relocation_vector}
\end{equation}
where $\Gamma_i$ is the coarse-grained tangency path bundle entering the local
observer cluster, $w_{ij}$ are tangency weights, $s_{ij}^a$ are edge vectors,
and $k_{\rm local}$ is the signed curvature scale of the observer's local
sphere cluster.  Exact local balance gives no net relocation; anisotropic
matter distributions or lensing paths generate nonzero $\xi^a$.

The effective relocation stress-energy is written as
\begin{equation}
  T_{\mu\nu}^{\rm reloc}
  =
  \rho\left(
  ...
  +\frac{1}{c^2}\xi_{(\mu}\nabla_{\nu)}\Phi
  \right),
  \label{eq:reloc_stress}
\end{equation}
where $u_\mu$ is the continuum four-velocity of the matter register and
$\Phi=c^2u$ is the radius-clock potential.  The associated weak perturbation
ansatz is
\begin{align}
  h_{0i} &=-\frac{2}{c^2}\xi_i,
  \label{eq:reloc_h0i}\\
  h_{ij} &=-\frac{2}{c^2}
  \left(\delta_{ij}\Phi+\partial_i\xi_j+\partial_j\xi_i\right).
  \label{eq:reloc_hij}
\end{align}
After imposing the harmonic-gauge condition associated here with coarse-grained
tangency balance, the conjectured tensor-sector continuum equation is
\begin{equation}
  \boxed{
  G_{\mu\nu}
  =
  \frac{8\pi G}{c^4}T_{\mu\nu}^{\rm reloc}.}
  \label{eq:einstein_relocation}
\end{equation}
Here $T_{\mu\nu}^{\rm reloc}$ is the apparent stress-energy density produced
by relocation through the observer's spherical lens.  It should not be read as
a claim that literal smooth spacetime curvature is fundamental.  Rather,
Eq.~\eqref{eq:einstein_relocation} is a continuum ansatz for the same bounded
observer effect that, in the scalar sector, appears as the radius-clock
potential.

In this interpretation, black holes are approximate outer boundaries of Core
Balls.  When tangency density around a Core Ball becomes high enough, info-ball
paths no longer escape the local register.  This threshold appears to a
continuum observer as an event horizon.  The model therefore replaces the
singularity with a dense, effectively self-contained tangency boundary.
...
Eq.~\eqref{eq:mobius_newton_prefactor} gives the exact Newtonian acceleration
term
\begin{equation}
...
On the ASSP graph the local proper tick is set by the local tangency step.
For an edge $(i,j)$,
\begin{equation}
  \Delta\tau_{ij}\sim\frac{R_i+R_j}{c}.
  \label{eq:tick_radius_edge}
\end{equation}
In a locally equal-radius coarse-grained region, $R_j\approx R_i$, this reduces
to
\begin{equation}
  \Delta\tau_i\sim\frac{2R_i}{c}.
  \label{eq:tick_radius}
\end{equation}
The mixing angle therefore becomes, in that local approximation,
\begin{equation}
...
\end{equation}
the continuum expansion of~\eqref{eq:assp_walk} is expected to give a
curved-space Dirac equation on the radius-clock metric.  With coordinates
$x^\mu=(t,x,y,z)$, a dimensionally explicit form is
\begin{equation}
  \boxed{
  \left[
  i\hbar c\,\gamma^a e_a{}^\mu
  \!\left(\partial_\mu+\Omega_\mu+\frac{iq}{\hbar}A_\mu\right)
  -mc^2
  \right]\Psi=0.}
  \label{eq:dirac_curved}
\end{equation}
Here $e_a{}^\mu$ is the tetrad of metric~\eqref{eq:metric}, with the time
component carrying the lapse factor $e^{-u}=R_0/R$; the spin connection
$\Omega_\mu$ is the continuum limit of inter-sphere frame rotations; and
$A_\mu$ arises from U(1) tangency edge phases.  This section is a continuum
ansatz constrained by the walk moments, not yet a full derivation of the spin
connection from the microscopic scattering matrices.  Thus the same graph that
produces gravity through its radius-clock field is proposed to produce quantum
matter through its directed spinor walk.
...
electromagnetism.  The tensor sector is represented by a visual-relocation
ansatz in which $G_{\mu
u}$ records apparent stress-energy generated by
spherical tangency-lens displacement rather than fundamental smooth curvature.

...
\begin{abstract}
We propose a new principle of physics---MATHICCS---requiring that every mathematical axiom used in a physical law must correspond to an operation persistently realizable by a physical subsystem. Applying this criterion motivates reality's geometry as a discrete, self-similar, singularity-free structure: the Apollonian Soddy Sphere Packing (ASSP). We conjecture that 3D dynamics on this packing is a \emph{self-computing fractal}: a dynamical computer whose registers are fractal-congruent objects that store angular and linear momentum as cross-scale registers. This fractal quine is proposed as a self-describing, MATHICCS-valid physical theory: it derives the fine-structure constant
\(\alpha^{-1}_{\rm TFOFT}=4\pi^3+\pi^2+\pi=137.036\)
from the minimal Presburger computational cost of a Soddy sphere state update, derives the 77-layer star-electron hierarchy from the rank-23 half-boundary mass register, derives the hydrogen binding energy from the Gyro Ball IO equation
\(E=\alpha^2m_ec^2/2\), and derives the gravitational constant \(G\) from atomic structure by two independent first-principles routes. We present empirical cross-checks involving the 25-arcminute CMB seam, Karlsson quasar logarithmic periodicity, and the observed electron/star hierarchy, together with one pending falsifiable prediction,
\(\alpha^{-1}_{\mathrm{Yb}}=137.035998945\pm20\,\mathrm{ppb}\). The framework is presented as a zero-free-parameter ontology whose derived consequences are compared against major tensions in observational physics.
\end{abstract}
...
\paragraph{Notation convention for derived and observed quantities.}
To prevent circular interpretation, this paper distinguishes ontology-internal
derived quantities from empirical comparison values. A superscript or subscript
\({\rm TFOFT}\) denotes a value derived internally from the Presburger-ASSP
ontology and its stated geometric rules. A superscript or subscript
\({\rm obs}\) denotes an observed astrophysical or cosmological value, while
\({\rm meas}\) denotes a laboratory measured value. Thus
\(k_{\rm mass}^{\rm TFOFT}\), \(T_{\rm mass}^{\rm TFOFT}\), and
\(S_{\rm mass}^{\rm TFOFT}\) are internal derived quantities, whereas
\(\ln(M_\odot/m_e)_{\rm obs}\), \(G_{\rm meas}\), and
\(\alpha_{\rm meas}\) are comparison quantities. Observed quantities may test
or calibrate the ontology, but they do not define the internal derivations
unless explicitly labeled as empirical inputs.
...
The mass hierarchy is not defined by the observed electron-to-solar mass ratio.
It is the self-encoding layer count of a Star Ball mass register consisting of
the full single-sphere state plus one half of the rank-23 curvature boundary
needed to define that state relationally. The observed electron-to-solar
hierarchy is therefore moved to a later cross-check: it tests whether the
internal closure lands near the expected astrophysical scale, but it is not an
...
137.0363038.
\]
This is the complete Presburger-ASSP state cost of one isolated Soddy sphere.
...
The 77-layer count follows from the fact that the Star Ball mass hierarchy is
a one-register hierarchy that still requires a relational boundary. It is not
the isolated single-ball limit and it is not the full two-ball quine transfer.
...
\begin{tikzpicture}[x=0.095cm,y=1cm,font=\small]
  % Axis scale: plot S-layer values from 76.65 to 77.35
  \def\xmin{76.65}
  \def\xmax{77.35}
  \def\Ssingle{76.7714}
  \def\Smass{77.0100}
  ...
  % helper macro for x coordinate
  \newcommand{\sx}[1]{({#1-\xmin})}

  \draw[->,thick] (0,0) -- ({\xmax-\xmin+0.08},0)
    node[right] {$S=T/k_{\rm mass}^{\rm TFOFT}$};

  \foreach \v/\lab in {76.7/76.7,76.8/76.8,76.9/76.9,77.0/77.0,77.1/77.1,77.2/77.2,77.3/77.3}
  {
    \draw ({\v-\xmin},0.06) -- ({\v-\xmin},-0.06);
    \node[below] at ({\v-\xmin},-0.06) {\scriptsize \lab};
  }

  % interval
  \draw[very thick] ({\Ssingle-\xmin},0.35) -- ({\Sfull-\xmin},0.35);

  % markers
  \draw[thick] ({\Ssingle-\xmin},0.15) -- ({\Ssingle-\xmin},0.55);
  \fill ({\Ssingle-\xmin},0.35) circle (2pt);
  \node[above,align=center] at ({\Ssingle-\xmin},0.62)
    {\scriptsize isolated\\[-2pt]\scriptsize single ball\\[-2pt]\scriptsize $76.771$};

  \draw[very thick] ({\Smass-\xmin},0.08) -- ({\Smass-\xmin},0.72);
  \fill ({\Smass-\xmin},0.35) circle (2.5pt);
  \node[above,align=center] at ({\Smass-\xmin},0.82)
    {\scriptsize half-boundary\\[-2pt]\scriptsize mass register\\[-2pt]\scriptsize $77.010\approx77$};

  \draw[thick] ({\Sfull-\xmin},0.15) -- ({\Sfull-\xmin},0.55);
  \fill ({\Sfull-\xmin},0.35) circle (2pt);
  \node[above,align=center] at ({\Sfull-\xmin},0.62)
    {\scriptsize full two-ball\\[-2pt]\scriptsize boundary\\[-2pt]\scriptsize $77.249$};

  % integer 77
  \draw[dashed] ({77.0-\xmin},-0.15) -- ({77.0-\xmin},0.78);
  \node[below] at ({77.0-\xmin},-0.32) {\scriptsize integer closure \(77\)};

  \node[align=center] at ({(77.0-\xmin)},1.55)
    {\normalsize Star Ball mass register sits between the isolated state and full two-ball transfer};
\end{tikzpicture}
\caption{Interval view of the TFOFT mass-register closure. The isolated
single-ball limit gives \(76.7714\) layer units; the full two-ball boundary
gives \(77.2486\) layer units. The half-boundary Star Ball mass register gives
\(S_{\rm mass}^{\rm TFOFT}=77.0100\approx77\), selecting the integer closure
without using the observed electron-to-solar mass ratio.}
\label{fig:mass_register_interval}
\end{figure}

\subsubsection{Observed Electron-to-Solar Hierarchy as a Cross-Check}
\label{sec:mass_observed_crosscheck}

Only after the internal mass-register closure is derived do we compare it with
observation. The observed electron-to-solar mass logarithm is
\[
\ln(M_\odot/m_e)_{\rm obs}
\approx
138.936.
\]
If the TFOFT-derived integer closure \(S_{\rm mass}^{\rm TFOFT}=77\) is used
as the layer count, this observed hierarchy corresponds to an observed
effective step
\[
k_{\rm mass}^{\rm obs}
=
\frac{\ln(M_\odot/m_e)_{\rm obs}}{77}
\approx
1.80436.
\]
This empirical value is not used in the derivation above. It is a comparison
quantity. The TFOFT-derived value is
\[
k_{\rm mass}^{\rm TFOFT}
=
1.7849921.
...
1.1\%.
\]
Under the present ontology this offset is interpreted as a local
astrophysical/substrate comparison residual, analogous in status to local
virial or density corrections appearing elsewhere in the paper. The important
logical point is that observation checks the internally derived 77-layer
closure; it does not define it.

\subsubsection{Karlsson/Quasar Periodicity as a Later Calibration, Not the Mass Derivation}
\label{sec:karlsson_mass_note}

Karlsson quasar logarithmic periodicity is not used here to derive the mass
step or the 77-layer count. In the present organization it may be retained
only as a later comparison, calibration layer, or phenomenological resonance
of the Info Ball channel. The primary mass hierarchy derivation is instead:
\[
\text{rank-23 geometry}
\;\Longrightarrow\;
k_{\rm info}^{(0),{\rm TFOFT}}
\;\Longrightarrow\;
k_{\rm mass}^{\rm TFOFT}
...
S_{\rm mass}^{\rm TFOFT}\approx77.
\]
This prevents the Karlsson value from functioning as a hidden observational
input to the Star Ball mass-register closure.
...
\subsection{Quasars as Candidate Info Balls and the Karlsson Logarithmic Periodicity}
...
\subsubsection{Karlsson Periodicity as an Empirical Info-Channel Check}

Karlsson quasar periodicity is treated here as an empirical comparison, not as an input to the mass-register derivation. Karlsson's reported logarithmic spacing is
\[
\Delta\log_{10}(1+z)_{\rm obs}=0.089.
\]
In natural logarithmic units this is
\[
...
The internally derived TFOFT Info-channel leakage is
\[
k_{\rm info}^{(0),{\rm TFOFT}}
=
2\left(\frac{23}{27}\right)^2\left(\frac{4}{27}\right)
...
0.2150079.
\]
Thus the observed Karlsson spacing lies near the internally derived Info-channel step:
\[
k_{\rm Karlsson}^{\rm obs}=0.2049301,
\qquad
k_{\rm info}^{(0),{\rm TFOFT}}=0.2150079.
\]
The fractional separation is
\[
\frac{
...
This near-match motivates the observational conjecture that quasars are candidate macroscopic Info Ball events. A quasar is therefore interpreted, within this conjecture, as a high-angular-momentum macrophoton-like transfer event associated with a parent macro-star entering or coupling to a galactic Core Ball. The conjecture is empirical: it is based on the proximity between an observed quasar logarithmic spacing and an independently derived TFOFT Info-channel scale.

Disputes about the statistical significance of Karlsson periodicity do not affect the primary TFOFT derivations of \(\alpha^{-1}_{\rm TFOFT}\), the rank-23 half-boundary 77-layer mass closure, or the two-route derivation of \(G\). In the present paper, Karlsson periodicity is used only as an observational Info-channel comparison.

\subsubsection{The Karlsson Complement as a Mass-Channel Spot Check}

If the observed Karlsson spacing is treated as an empirical candidate for the Info-channel step, then its \(e\)-clamped complementary mass-channel scale is
\[
...
1.7950699.
\]
Over the TFOFT integer closure \(S=77\), this gives
\[
...
The CODATA/solar electron-star logarithmic hierarchy is
\[
N_{\rm mass}^{\rm obs}
=
\ln(M_\odot/m_e)_{\rm obs}
...
138.93575.
\]
Thus the Karlsson-complement mass channel lands in the same register interval between the isolated single-sphere value and the rounded two-ball boundary:
\[
137.0363
<
138.2204
...
139.
\]
The offset between the Karlsson-complement check and the observed electron-star hierarchy is
\[
138.93575-138.2204
=
0.7154,
...
This is not used to derive \(S_{\rm mass}^{\rm TFOFT}\). It is a spot check: the observed Karlsson Info-channel candidate has an \(e\)-clamped complement whose 77-layer mass-channel product falls inside the same 137--139 register window as the CODATA/solar hierarchy.

For comparison, the full TFOFT two-ball transfer register is
\[
...
138.9550.
\]
This lies very close to the observed electron-star hierarchy:
\[
T_{\rm two\text{-}ball}^{\rm TFOFT}
-
N_{\rm mass}^{\rm obs}
...
0.0193,
\]
a fractional difference of approximately
\[
1.4\times10^{-4}
=
0.014\%.
...
The hierarchy of checks is therefore
\[
\begin{aligned}
T_{\rm sphere}^{\rm TFOFT} &= 137.0363,\\
77k_{\rm mass|Karlsson}^{\rm obs} &= 138.2204,\\
\ln(M_\odot/m_e)_{\rm obs} &= 138.93575,\\
T_{\rm two\text{-}ball}^{\rm TFOFT} &= 138.9550.
\end{aligned}
\]
All four values sit in the same register interval, with the full two-ball boundary providing the closest match to the CODATA/solar hierarchy.

...
\label{sec:alpha_deriv}
  The fine-structure constant is not introduced in the theory as an empirical parameter. It is derived as the elementary probability of the Presburger--ASSP substrate. The inverse value, \(\alpha^{-1}\), is the full causal register depth required to complete an internally self-consistent Soddy-sphere update.
  \begin{quote}
    \textbf{Axiom P4 (Elegant Optimized Code):}
    The operational rules of 
    self-describing Presburger-ASSP machine minimize information cost subject t
    MATHICCS constraints. Since the physical registers are literal spheres, th
    radius, circumference, surface, rotation, and displacement channels must b
    encoded using the same reusable geometric word whenever possible. Th
    Presburger-minimal basis radius is therefore forced to be \(r=\pi\): choosin
    \(r=1\) introduces an additional lookup between the register's geometric wor
    and its rotational/surface channels, while \(r=\pi\) collapses all radiu
    references into a single persistent symbol. This is not a numerical fit but 
    minimal-code consequence of Axiom P2 under the ASSP substrate
  \end{quote}

  A complete Soddy-sphere register update must specify three irreducible state channels:

  \[
  T_S = 4\pi r^2,
  \]

  the surface/specification channel,

  \[
  T_R = \pi r,
  \]

  the hemisphere-restricted rotational phase channel, and

  \[
  T_D = r,
  \]

  the radial displacement channel.

  By Axiom P4, the Presburger-minimal basis radius is forced to be

  \[
  r=\pi.
  \]

  Therefore the three channel costs become
  ...
  The total Presburger cost to fully specify one Soddy sphere's dynamical state is the sum of these three channel costs:
  ...
  The quantity \(T_{\rm sphere}\) is not merely a static information count. Within the Presburger-ASSP ontology it is the number of internally distinguishable causal micro-positions in one complete Soddy-sphere register update.  A complete register update must preserve the ordered surface channel, the hemisphere-restricted rotational phase channel, and the radial displacement channel simultaneously.

  The Presburger-ASSP machine has no external synchronizing clock. Neighboring ball-registers therefore update asynchronously.  An elementary electromagnetic transaction is defined, in this ontology, as the successful phase-aligned causal handshake between two adjacent Soddy registers: one register emits or presents a boundary phase defect, and the neighboring register accepts it in the causally ordered phase position compatible with the full single-sphere update.
  ...
  This follows because a successful adjacent-register handshake must close the surface, rotational-phase, and radial-displacement channels in the same causal order.  Any offset in the receiving register fails to preserve the full two-ball transaction.  Therefore, among the \(T_{\rm sphere}\) internally distinguishable causal positions of the receiving register, exactly one position accepts the incoming boundary phase defect.

  Hence the elementary coupling probability is not introduced phenomenologically.  It is the finite-state counting probability for a unique accepting alignment inside a register with \(T_{\rm sphere}\) possible causal positions:
  ...
  Since the electromagnetic channel is precisely this adjacent-register
  phase-alignment channel, the fine-structure constant is the elementary
  ...
  Thus \(\alpha^{-1}\) is the full causal register depth of one complete
  Soddy-sphere update, while \(\alpha\) is the probability that an asynchronous
  neighboring register lands in the unique accepting alignment required for an
  elementary electromagnetic transaction.  The identification of
  \(P_{\rm align}\) with \(\alpha\) is therefore deductive inside the ontology:
  electromagnetism has already been defined as adjacent-register phase transfer,
  and \(\alpha\) is the elementary probability for such transfer.

  \paragraph{Physical interpretation.}

  The inverse fine-structure constant measures the full causal throughput cost
  for one complete Presburger tick of a single Soddy sphere:
  ...
  This throughput cost includes surface specification, rotational phase, and
  radial displacement.  An electromagnetic interaction is the adjacent-register
  phase-transfer transaction between two causally neighboring spheres.  Because
  the transaction has exactly one accepting alignment among \(T_{\rm sphere}\)
  possible internal causal positions, its elementary throughput probability is
  ...
  No CODATA value is used as an input.

  The value derived here is not obtained by fitting the CODATA recommended
  least-squares value of \(\alpha\)~\cite{CODATA2022}.  It is the
  ontology-internal ASSP register value.  Effective low-energy determinations of
  \(\alpha\) may differ between experimental channels because each atom realizes
  the electromagnetic register through a different evolved local configuration.
  This distinction is important because the present atom-recoil situation is not
  perfectly settled: high-precision atom-recoil determinations using cesium and
  rubidium differ at the several-sigma level.  The 2018 cesium measurement gave
  ...
  least-squares CODATA input.  The underlying ASSP alignment probability is
  invariant; experimentally inferred values probe local atomic realizations of
  that invariant register channel.
...
\subsection{Derivation of the Electron Rest Mass and Proton Mass from Recursive Soddy Curvature}
...
The factor \(23/27 \approx 0.85185185\) is the luminous (mass-bearing) channel fraction. It corresponds to the Strassen-inspired heavy, curvature-intensive portion of the two-ball gyro-ball interaction that builds persistent inertial registers. The complementary \(4/27\) channel is the low-cost Info Ball transfer.

...
Wealth distribution (Pareto tail) & $1.5$--$3$ & $\approx1.89$--$\approx2.47$ & Apollonian/Soddy \\
...
Equivalently, \(T_{\rm sphere}\approx137\) is not merely a linear count. It is the logarithmic depth of the causal register. A stable self-similar register with \(T_{\rm sphere}\) finite causal update positions therefore spans an exponential state range
\[
  \mathcal{N}_{\rm sphere}
  \sim
  e^{T_{\rm sphere}}
  \approx
  e^{137}.
\]
Thus the derivation of \(\alpha^{-1}\) also fixes the natural single-sphere exponential register size. The value \(137\) is the finite Presburger update cost; \(e^{137}\) is the corresponding causal state capacity across recursive fractal layers.
\end{mdframed}
\end{samepage}
...
The ratio is
\[
\frac{4.96\times10^{33}}{2.7585\times10^{33}}
\approx
1.80.
\]
...
1.8898.
\]
Thus the corrected atom-to-galaxy orbital hierarchy misses the \(77\)-layer register value by a factor of order the mixed Apollonian--Soddy boundary--bulk dimension, rather than by an arbitrary scale. The important point is the ordering: \(c_{\rm wish}\sim10^2c_{\rm here}\) is derived first from the Sol--SgrA* macro-hydrogen Bohr relation, and only then used as a correction to the observed \(10^{31}\) atom-to-galaxy orbital hierarchy. The corrected hierarchy lands on the same \(10^{33}\) scale as \(e^{77}\), providing an independent register-depth cross-check rather than a fitted normalization.

This subsection is not used to derive the \(77\)-layer mass register. That register was already obtained internally from the rank-23 half-boundary construction. The purpose here is different: the Sol--SgrA* macro-hydrogen relation supplies an independent cross-layer light-speed factor, and that factor carries the observed atom-to-galaxy orbital hierarchy onto the same \(e^{77}\) exponential band. The agreement is therefore a cross-scale structure check, not a normalization fit.
...
The following table summarizes the major observational tensions addressed by
the completed FTC ontology. The entries are stated as structural resolutions
inside TFOFT, not as independent proof of the ontology.

\begin{center}
\small
\renewcommand{\arraystretch}{1.28}
\begin{longtable}{|L{4.4cm}|L{9.7cm}|}
\caption{Resolved tensions under the completed FTC ontology.}
\label{tab:resolved_tensions_ftc}\\
\hline
\textbf{Tension or anomaly} & \textbf{FTC resolution} \\
\hline
\endfirsthead
\hline
\textbf{Tension or anomaly} & \textbf{FTC resolution} \\
...
Hubble tension &
Redshift has a dominant Brownian boundary-scattering component plus a secondary coherent-motion component. Distance ladders weight these components differently, producing inconsistent apparent \(H_0\) values without changing the underlying FTC geometry~\cite{Riess2025}. \\
\hline

JWST early massive galaxies &
High observed redshift measures accumulated boundary crossings, parent-well depth, and inherited core material rather than simple cosmic age. Mature galaxies at high \(z\) are recycled Core/Star material relaxing after ejection from a parent macro-structure~\cite{Naidu2025}. \\
\hline

Quasar dipole / CMB dipole offset &
Quasars are post-Info-Ball transport objects. Core-boundary injection carries angular momentum across a M\"obius crossing, forcing a near-orthogonal orientation relative to ordinary galaxy motion; the residual few degrees are finite-shuffling error~\cite{Secrest2022,Secrest2025}. \\
\hline

CMB blackbody and acoustic peaks &
The CMB is the steady resonant heat bath of the lower-fractal-layer hydrogen substrate. Acoustic peaks are active pressure modes of the present FTC medium, not a required singular-origin relic~\cite{Planck2018VI}. \\
\hline

CMB Cold Spot, birefringence, and 25-arcminute seam &
A local Null Core / void-sphere boundary imposes one dominant M\"obius twist and a minimal tangency angle \(\theta_c=1/\alpha^{-1}\,\mathrm{rad}\approx25.08'\). The same geometric seam appears in Cold Spot boundaries, dipole nulls, and polarization rotation~\cite{Planck2018VI,Minami2020}. \\
\hline

\(S_8/\sigma_8\) clustering tension &
Matter clustering statistics depend on local effective fractal dimension \(D_{\rm eff}\). Low-redshift lensing probes post-shuffled dense substrate while the CMB probes the smoother baseline packing. The mismatch is a finite-shuffling signal rather than a universal-growth failure. \\
\hline

Baryon inventory scale &
The 137-layer single-sphere register gives \(4^{137}\approx3.04\times10^{82}\), landing in the observed baryonic-inventory exponent band. This is the global-count analogue of the same register depth that gives \(\alpha^{-1}\)~\cite{Planck2018VI,Page2016,Macquart2020}. \\
\hline

Cosmological constant / vacuum-energy problem &
The vacuum is the lower-layer ASSP substrate, not a continuum field carrying a freely summed zero-point energy. Energy is bounded layer by layer by the finite FTC register budget and bleeds through upper/lower scale boundaries. \\
\hline

Dark matter flat rotation curves &
Dark matter is lower-scale hydrogen substrate supporting the here-scale Star/Core system. Flat rotation curves are the large-scale projection of the same lower-layer bound support that appears as electron-cloud structure at atomic scale. \\
\hline

Dark/visible fraction &
The Laderman rank-23 split supplies the finite overhead fractions: \(23/27\) dark/support channel and \(4/27\) luminous/visible leakage channel. The observed dominance of dark support is therefore a boundary-compute ratio, not an added particle sector~\cite{Laderman1976}. \\
\hline

Dark matter--neutrino interactions &
Neutrinos are Code Balls, i.e. neutral substellar lifecycle channels. Dark matter is lower-scale hydrogen substrate. Their interaction is mechanical drag, accretion, stripping, and lifecycle exchange through lower-scale gas, with no new here-scale mediator particle. \\
\hline

Neutrino masses and no fourth light neutrino &
The three neutrino flavor-channel ceilings align with the three passive substellar regimes: rocky, gas/ice giant, and brown dwarf. A fourth light neutrino would require a fourth stable passive substellar lifecycle band between brown dwarf and active Star Ball, which the FTC dictionary does not supply~\cite{KATRIN2025,PDG2024,ALEPH1998TauNu,Spiegel2011}. \\
\hline

Muon and tau instability &
The muon/electron mass ratio maps to an overmassive \(\sim207M_\odot\) emitting Star Ball channel. The tau/electron ratio maps to a compact \(\sim3477M_\odot\) cluster-scale emitting channel. Their short lifetimes express unstable over-depth registers relaxing back toward lighter emitters plus neutral Code Ball channels. \\
\hline

Hierarchy problem &
The electroweak/Planck hierarchy is replaced by the 77-layer self-encoding depth of the Star Ball mass register. The hierarchy is the finite number of recursive FTC layers needed for a stable quine, not a fine-tuned scalar-sector accident. \\
\hline

Strong CP problem / neutron EDM &
The strong channel is lower-scale gravity confined inside the Core/Pipe boundary. Triple-tangency Core geometry is M\"obius-symmetric, so no preferred CP orientation is available at leading order. \\
\hline

Quark confinement and gluons &
Galaxy cores / proton Core Balls are irreducible FTC absorbers at their layer. Quark labels and gluons are Standard Model internal coordinates for Core response and confinement bookkeeping. The physical binding channel is lower-scale gravity inside the Core/Pipe boundary. \\
\hline

Electroweak unification &
The weak interaction is lower-scale local electromagnetism bleeding through a boundary channel. Electroweak unification follows because both are the same phase-transfer force viewed across adjacent register depths. \\
\hline

Wavefunction collapse and Born rule &
Measurement is a Core--Star boundary handshake. Resolution occurs when the causal update sequence reaches the unique accepting alignment \(N_{\rm accept}=1\). The Born-rule counting probability is the same finite-register counting structure used for \(\alpha\). \\
\hline

Black-hole information problem &
A black hole is a Core Ball boundary crossing. Information changes fractal register through M\"obius inversion and lower-layer Dust Ball production; angular momentum is conserved across the boundary by construction. \\
\hline

GR singularities and EEP at future infinity &
MATHICCS excludes point singularities and non-persistent limiting operations. The ASSP has finite recursive cells at every realized layer, while the EEP limiting procedure has finite constructive support in a \(\Lambda>0\) universe. \\
\hline

Fermi bubbles &
The vertical extent follows the \(23/27\) support channel, and the lobe angle follows the \(3d_{z^2}\) nodal-cone geometry. Both emerge from the same FTC boundary and orbital-channel structure~\cite{Su2010,Dobler2010,Ackermann2014,Sarkar2015}. \\
\hline

Karlsson quasar periodicity &
Discrete Core-crossing events produce logarithmic redshift steps. The observed Karlsson spacing is the quasar-scale projection of the Info Ball leakage channel~\cite{Karlsson1971,Mal2020}. \\
\hline

AGN jet power and bipolarity &
Jets are non-local Info Ball channels entering and exiting Core/Pipe boundaries. Bipolarity follows from M\"obius pole structure; jet power need not be supplied solely by local accretion or black-hole spin. \\
\hline

Ohmic resistance and CISS &
Electrical transport is lower-scale Star Ball disruption/reconstruction through atomic Pipe channels. Chirality-induced spin selectivity is a real boundary-geometry effect of handed tangent chains, not a perturbative afterthought added to a featureless continuum. \\
\hline
...
The common pattern is finite register leakage across adjacent scale boundaries.
TFOFT therefore treats many apparent anomalies as different measurements of the
same object: a self-computing ASSP whose local channels are seen as particles,
forces, redshift components, dark substrate, or thermal background depending on
which register boundary the observer probes.
...
\begin{enumerate}
  ...
  from the same finite register architecture:
    \begin{itemize}
      ...
      \item \(4^{137}\approx3.04\times10^{82}\), the 137-generation
      four-way Soddy inventory count, landing in the observable-universe
      baryonic inventory exponent band~\cite{Planck2018VI,Page2016,Macquart2020}.
  ...
  different projections of the same FTC substrate dynamics. A compact map is
  given in Section~\ref{sec:resolved_tensions}.

...
four stages. First, we show that GR's foundational assumption---the Einstein
...
than emergent, we derive the explicit boundary-crossing rules (M\"obius inversion plus
strict angular-momentum conservation) that produce universal bipolar jets and
quantum/galactic non-locality from a single mechanism. Fourth, cross-void quasar
correlations appearing superluminal from $S{=}0$ establish a true cosmic distance
ladder that resolves the Hubble tension categorically, while sparse-space attraction
to dense space unifies gravity and light as two descriptions of the same dense-sparse
...
ds^2 = -\left(1 - \frac{2GM}{r} - \frac{\Lambda r^2}{3}\right) dt^2 +
       \frac{dr^2}{1 - \frac{2GM}{r} - \frac{\Lambda r^2}{3}} + r^2 d\Omega^2.
\end{equation}
The horizons are roots of $f(r) = 1 - \frac{2GM}{r} - \frac{\Lambda r^2}{3} = 0$,
with $r_{\rm BH} < r_{\rm c}$ defining the static patch $r_{\rm BH} < r < r_{\rm c}$
where $f(r) > 0$.

\subsection{Collapse Regions: Observers Fall to Singularities}

In the SdS static patch $r_{\rm BH} < r < r_{\rm c}$, realistic observers experience
orbital decay via gravitational radiation and tidal interactions. All bound trajectories
terminate at the black hole singularity in finite proper time $\tau$, so the physical
radial separation satisfies
\begin{equation}
\lim_{\tau \to \tau_{\rm sing}} \Delta r_{\rm phys}(\tau) = 0, \qquad \Delta r = r_{\rm c} - r_{\rm BH} \sim \sqrt{3/\Lambda} - 2GM.
\end{equation}

Define the indicator functional for a spacetime region $\mathcal{R}$ at coordinate time $t$:
\begin{equation}
F_{\rm collapse}(\mathcal{R}, t; L_{\rm phys}, \varepsilon) =
\begin{cases}
1 & \text{if $\mathcal{R}$ contains a finite lab of size $L_{\rm phys}>0$ sustaining relative motions $> \varepsilon L_{\rm phys}$}, \\
0 & \text{if all observers in $\mathcal{R}$ have collapsed ($\Delta r_{\rm phys} < \varepsilon L_{\rm phys}$)}.
\end{cases}
\end{equation}
For fixed laboratory precision $\varepsilon > 0$ (e.g., $10^{-6}$ for accelerometers) and
fixed $L_{\rm phys} > 0$ (e.g., 1~m), we have $F_{\rm collapse}(t_{\rm finite})=1$ but
$\lim_{t\to\infty} F_{\rm collapse}(t)=0$. This holds despite smooth Einstein PDE solutions.

The 3-volume fraction of EEP-valid subregions within the static patch therefore obeys
\begin{equation}
...
In asymptotic de Sitter voids ($a(t) = e^{Ht}$, $H=\sqrt{\Lambda/3}=$ constant), free massive
particles suffer exponential peculiar momentum decay:
\begin{equation}
p_{\rm pec}(t) = p_0 e^{-Ht}, \quad v_{\rm pec}(t) = v_0 e^{-Ht} \to 0.
\end{equation}
All observers become perfectly comoving at fixed comoving coordinates $\chi_\infty$,
frozen relative to the Hubble flow with Hubble radius $R_H = c/H$.

The EEP requires finite-size labs ($L_{\rm phys}>0$) capable of controllable internal dynamics
(accelerations, signals across $L_{\rm phys}$). But with $v_{\rm pec}\to 0$:
\begin{itemize}
\item No relative motion between test masses ($\Delta v < \varepsilon L_{\rm phys}/\tau_{\rm lab}$).
\item No ability to accelerate or send signals across finite distances in finite proper time.
\item EEP ``inertial patch'' shrinks below $L_{\rm phys}$.
\end{itemize}

Define
\begin{equation}
F_{\rm void}(\text{lab at }t; L_{\rm phys}, \varepsilon) =
\begin{cases}
1 & \text{if lab sustains EEP dynamics ($v_{\rm pec}(t) > \varepsilon L_{\rm phys}/\tau_{\rm lab}$)}, \\
0 & \text{if frozen comoving (no internal motion)}.
\end{cases}
\end{equation}
Then $F_{\rm void}(t_{\rm finite})=1$ but $\lim_{t\to\infty} F_{\rm void}=0$. The spatial
volume fraction of valid EEP labs scales as
\begin{equation}
f_{\rm void}(t) \sim \left(\frac{v_{\rm pec}(t) \tau_{\rm lab}}{L_{\rm phys}}\right)^3 \sim e^{-3Ht} \to 0.
\end{equation}

\subsection{The $t\to\infty$ Bifurcation}

Spacetime bifurcates into complementary failure modes, each yielding $F=0$:
\begin{itemize}
    \item \textbf{Bound regions (SdS/static patches)}: $\Delta r_{\rm phys}\to 0$ via collapse, $f_{\rm collapse}\to 0$.
    \item \textbf{Void regions (de Sitter)}: $v_{\rm pec}\to 0$ via redshift freeze, $f_{\rm void}\to 0$.
\end{itemize}

The global EEP indicator is
\begin{equation}
F_{\rm EEP}(\text{event }p; L_{\rm phys}, \varepsilon) =
\begin{cases}
1 & \text{if $p$ admits finite-size lab ($L_{\rm phys}$, precision $\varepsilon$) with controllable dynamics}, \\
0 & \text{otherwise}.
\end{cases}
\end{equation}
On proper-time slices $\Sigma_t$ for comoving observers, the 4-volume measure obeys
\begin{equation}
\int_{\Sigma_t} F_{\rm EEP} \, \sqrt{-g} \, d^3x \to 0 \quad (t\to\infty),
\end{equation}
almost everywhere. Pointwise Riemann normal coordinates exist, but their physical domain shrinks to measure zero.

\subsection{The ``1=0'' Pathology}

Pure GR on a smooth manifold remains formally consistent: smooth metrics, valid Einstein PDEs, pointwise EEP.
Yet the physically relevant content---finite labs ($L_{\rm phys}>0$, $\varepsilon>0$ fixed) performing
experiments---exhibits a discontinuous jump:
\begin{equation}
F_{\rm finite}(t; L_{\rm phys},\varepsilon) = 1 \quad \to \quad \lim_{t\to\infty} F(t; L_{\rm phys},\varepsilon) = 0.
\end{equation}
This functional discontinuity signals that classical GR's domain of physical applicability
(EEP labs) occupies vanishing 4-volume measure in the infinite-time limit.

The interface between collapse and void regions---event horizons and descendants---carries
the remaining structure. No smooth manifold covers this boundary while preserving finite-size
EEP labs, necessitating a degenerate Apollonian sphere packing (Section~\ref{sec:apollonian_geometry}).
...
the 3D Apollonian gasket is $D \approx 2.47$ \cite{Borkovec1994}---not an integer, not a smooth manifold,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Core Equations of the Fractal Scale Equivalence Principle}
...
The curvature $k_5$ of a sphere tangent to four mutually tangent spheres is
\begin{equation}
...
Scale coordinate (observer locked at $s=0$):
\begin{equation}
...
\label{eq:density}
\end{equation}
Dense core when $\delta(V)\ge\delta_{\rm crit}(s)$ (infall dominates). Sparse/void core when $\delta(V)<\delta_{\rm crit}(s)$.

At every tangency boundary of radius $R$, center $O$:
\begin{enumerate}
\item Spherical inversion:
\begin{equation}
\mathbf{r}' = O + \frac{R^2 (\mathbf{r} - O)}{|\mathbf{r}-O|^2}.
\end{equation}
\item M\"obius map on sphere: $\theta' = \theta + \pi$, $\phi' = -\phi$.
\item Scale flip:
\begin{equation}
\mathbf{r}'' =
\begin{cases}
...
\end{cases}
\end{equation}
\item Strict conservation of angular momentum $\mathbf{L} = \mathbf{r} \times \mathbf{p}$ w.r.t.\ $O$:
\begin{equation}
\mathbf{L}' = \mathbf{r}'' \times \mathbf{p}' = \mathbf{L} \quad\Rightarrow\quad
...
Star--photon--star loop across one void core:
\begin{align}
\Delta t &= D_{\rm void}/c, &
\mathbf{L}_{\rm global} &= \text{constant}, &
E' &= E \cdot (r_{\rm out}/r_{\rm in}).
\label{eq:loop}
\end{align}

These equations, together with the three co-axioms (fractal fundamental, sparse attracted to dense, $\mathbf{L}$ conserved), constitute the complete microscopic engine of FSEP. All predictions follow directly from substituting local geometry into \eqref{eq:Lcons}--\eqref{eq:loop}.

(The simple photonic-billiards model is the stochastic Monte-Carlo implementation of the same rules inside each Apollonian ``BIG BALL'' $B_s$.)
...
that occurs at every fractal scale crossing (see Section~\ref{sec:core_equations}).
...
universe, just as all photons race outward from atoms to get into the rest of the
universe.
\end{quote}
...
dense--sparse boundary, implemented by the core transformation rules of Section~\ref{sec:core_equations}.
An observer at maximum angular momentum of their fractal scale
...
\qquad \lambda_\text{local} \equiv \frac{R}{a}.
\end{equation}
...
consistent with the M87* jet without any microphysical tuning \cite{Nikonov2023}. Bipolarity and
...
matter relative to isolated-spiral predictions \cite{Jiao2023}.
...
\begin{itemize}
    ...
    wall sends $S{=}+1$ stellar material across the void. From our $S{=}0$ perspective,
    the correlation between the two AGN events appears \textbf{superluminal}: the lag
    is far shorter than $D_\text{void}/c_{S=0}$ because the transit occurs at $S{=}+1$.
    This is the key distinguishing prediction from any conventional correlated-AGN model,
    which would predict a lag consistent with light-travel time across the void diameter.
    The jet axes of the two paired galaxies may be rotationally offset relative to the
    void-wall normal by an angle given by eq.~\eqref{eq:jet_angle}, providing a geometric
    correction that observers must account for when identifying paired systems.
...
\item Quasar B responds (AGN flaring, jet reorientation) with a lag
$\tau = D_\text{void}/c_{S+1}$ at the $S{=}+1$ level.
\item From our $S{=}0$ perspective, the correlation appears superluminal: the observed
lag $\tau_\text{obs} \ll D_\text{void}/c_{S=0}$. This is not a violation of causality;
it is causality operating at the adjacent fractal scale.
\item The ratio $\tau_\text{obs} / (D_\text{void}/c_{S=0})$ directly measures the
scale ratio $\lambda$ between $S{=}0$ and $S{=}+1$, providing an independent
calibration of the fractal ladder.
\end{enumerate}
...
will show correlated variability with apparent lags $\tau_\text{obs}$ far shorter than
$D_\text{void}/c_{S=0}$, with the degree of superluminality scaling with void diameter
and providing a direct measurement of $\lambda$. This is testable against existing
multi-epoch quasar catalogs cross-matched against void catalogs.
...
one part in $10^5$, with temperature $T \approx 2.725$~K \cite{Planck2014}. In $\Lambda$CDM, this is
...
Before presenting the mechanism, we note the foundational question that $\Lambda$CDM
leaves unresolved: \textit{in classical and quantum field theory, the vacuum has no
temperature; temperature requires a material substrate to radiate}. The CMB
temperature must therefore be the temperature \textit{of something}. In FSEP, that
something is identified precisely.

\medskip
\noindent\textbf{Step 1: What fills the voids.}
In FSEP, the fractal layer immediately below $S{=}0$ is composed of photons---
relativistic plasma ejected from hydrogen fusion events at stellar cores and dispersed
through galactic jets into the surrounding voids. These are not classical electromagnetic
waves in the sense of smooth-manifold field theory. They are $S{=}{-1}$ stellar plasma:
the sub-scale analogue of stars at $S{=}0$, produced continuously by fusion activity
and ejected through the M\"obius boundary into the inter-galactic voids. The voids are
not empty. They are filled with this sub-scale plasma, everywhere, continuously
replenished by the fusion activity of $S{=}0$ stars.

\medskip
\noindent\textbf{Step 2: Orphaned photons forming micro-stars.}
This sub-scale plasma in the voids is not inert. Following the same three axioms that
govern all fractal scales---sparse attracted to dense, angular momentum conserved,
fractal fundamental---the $S{=}{-1}$ plasma in the voids self-interacts. Sparse
sub-scale plasma clumps toward dense sub-scale plasma. The result is spontaneous
coalescence into new coherent structures: clusters of $S{=}{-1}$ plasma that are,
at their own scale, the exact equivalent of hydrogen stars at $S{=}0$. These
\textbf{micro-stars} form throughout the voids, independent of any $S{=}0$ galactic
structure, by the same gravitational coalescence mechanism that forms stars from
interstellar gas---just one fractal layer down. The voids are populated with a
continuous distribution of these self-organized micro-stellar objects, filling what
appears from $S{=}0$ to be empty space.

\medskip
\noindent\textbf{Step 3: The resonant harmonic and why the spectrum is perfect.}
These micro-stars undergo the same hydrogen fusion process as $S{=}0$ stars. Their
emission---the $S{=}{-1}$ equivalent of stellar radiation---propagates upward through
the fractal boundary into $S{=}0$ as the background microwave field we observe.

The critical point is that the emission wavelength is \textit{not} set by a
statistical temperature average over a distribution of sources at varying distances
and densities, which would produce a modified blackbody with measurable deviations.
Instead, it is set by the \textbf{fractal resonance condition}: the characteristic
emission wavelength of micro-hydrogen fusion at $S{=}{-1}$ maps to $S{=}0$ via the
scale ratio $\lambda$, landing at the harmonic frequency of the $S{=}0$ hydrogen
fractal layer boundary. A resonant harmonic is sharp by construction---this is why
the CMB spectrum is a perfect blackbody to one part in $10^4$. It is an eigenmode
of the fractal substrate, not a thermal average.

This identification is confirmed by direct observation. The wavelength $\sim 2$~mm
is precisely where stellar H$^+$ emission peaks in real star-forming regions:
the H26$\alpha$ through H30$\alpha$ radio recombination lines, produced by
high-frequency transitions at low principal quantum numbers, are detected as
emission lines in the submillimeter/millimeter range toward compact H\,\textsc{ii}
regions and high-mass star-forming clumps in the ATLASGAL survey \cite{ATLASGAL2014},
observed with APEX and ALMA. The CMB peak wavelength is not a coincidence of
cosmological history---it is the characteristic emission wavelength of hydrogen
stellar fusion plasma, arriving from the fractal layer below.

\medskip
\noindent\textbf{Isotropy from geometry, not causal contact.}
The CMB is isotropic to one part in $10^5$ in every direction on the sky.
$\Lambda$CDM requires a separate mechanism (inflation) to explain why causally
disconnected regions of the early universe happen to share the same temperature.
In FSEP, isotropy follows automatically from the geometry: \textit{the source is
not distant---we are immersed inside it}. The micro-stellar plasma fills the voids
uniformly by the same fractal symmetry that makes the Apollonian packing
space-filling. We are inside the substrate; it radiates toward us equally from
all directions by construction. No causal contact between distant regions is
required because there are no distant regions involved---the source is local
and omnidirectional.

\medskip
\noindent\textbf{Olbers' paradox dissolved.}
The night sky is dark because photons above the $S{=}0$ fractal resonance wavelength
interact with the substrate and scatter, losing energy until they reach the
transparent floor at $\sim 2$~mm. The CMB \textit{is} the floor: the resonant
emission of the fractal layer below at the wavelength to which the substrate is
transparent. This is not a redshift argument; it is a harmonic argument. The sky
is dark at optical wavelengths because those photons are actively scattering;
the sky is bright at 2~mm because that wavelength passes through the fractal
layer like butter---the H$_2$ gravitational collapse threshold at $\sim 0.1$~mm
sets the lower bound; the CMB floor at $\sim 2$~mm sets the upper bound of the
transparent window.

\medskip
\noindent\textbf{The CMB dipole as a first-principles prediction.}
The observed dipole ($\Delta T/T \sim 10^{-3}$, $v \approx 370$~km/s toward Leo
\cite{Lineweaver1996, Planck2014}) is not merely our local peculiar motion. It is a literal
geometric arrow pointing back to the star our hydrogen was fused in. Every hydrogen
atom in our instruments was synthesized in a stellar interior and ejected radially
outward. Our entire gas clump has been coasting outward from that stellar ejection
event ever since, at 370~km/s, with the ``hot'' CMB direction pointing back toward
the parent star's position in the Milky Way. In $\Lambda$CDM this is an unexplained
coincidence of local motion. In the FSEP it is the expected kinematic signature of
hydrogen atoms that carry the memory of their origin.

The residual anisotropies at $\Delta T/T \sim 10^{-5}$ are real density variations
in the local micro-stellar plasma distribution---structure in the present substrate---
not amplified quantum fluctuations of an inflationary field.

\medskip
\noindent\textbf{Falsifiable prediction.}
FSEP predicts that CMB spectral distortions, if detected at sufficient sensitivity,
will cluster at frequencies corresponding to \textit{fractal subharmonics} of the
2~mm hydrogen recombination line---not at the $\mu$- and $y$-distortion frequencies
predicted by $\Lambda$CDM's thermal history. This is testable against future
high-sensitivity CMB spectral distortion measurements with PIXIE-class instruments.
...
after the Big Bang \cite{Huang2024}. In the FSEP, high redshift measures high cumulative scattering
...
atomic scale, which the FSEP predicts should be derivable from the local $\lambda_\text{local}$.
...
\subsubsection{Cross-Void Quasar Correlations as Superluminal from $S{=}0$}
\label{sec:superluminal_pred}
Paired quasars across well-characterized supervoids (SDSS, DESI) will show correlated
AGN variability with observed lags $\tau_\text{obs}$ far shorter than the classical
light-travel time across the void diameter $D_\text{void}/c_{S=0}$. The degree of
apparent superluminality scales with void diameter and directly measures the fractal
scale ratio $\lambda$ between $S{=}0$ and $S{=}+1$. This is the primary
distinguishing prediction from all conventional correlated-AGN models, which predict
lags consistent with or longer than $D_\text{void}/c$. It is testable with existing
multi-epoch quasar catalogs cross-matched against void catalogs from SDSS and DESI,
...
\label{sec:baryon}
A foundational question for the FSEP cross-scale correspondence is the apparent status
of baryon number conservation. In standard particle physics, baryon number is a
globally conserved quantum number: no process within the Standard Model creates or
destroys net baryon number. The FSEP does not violate this conservation law---it
reframes its domain of applicability.

The key is the star-photon correspondence established in Section~\ref{sec:same_motion}:
\begin{equation}
\{S{=}0,\;\text{star}\} \;\longleftrightarrow\; \{S{=}+1,\;\text{photon}\}
\end{equation}
A photon at $S{=}+1$ is not a massless, baryon-free quantum. It is stellar matter---
a baryon-carrying object---viewed from the scale above. When such an $S{=}+1$ photon
crosses the M\"obius boundary and manifests at $S{=}0$, the baryons it carries are
not created; they arrive from $S{=}+1$ as the fractal boundary is crossed. Baryon
number is conserved \textit{across the full fractal hierarchy} taken as a whole.
What standard physics identifies as a conservation law holding within a single scale
layer is the projection of a deeper cross-scale conservation onto that layer alone.

This reframing has a direct experimental consequence. A laboratory test that appeared
to generate new baryonic material---hydrogen atoms appearing in a system where none
were present and where no classical source exists---would not be evidence against
baryon conservation. It would be evidence that baryon-carrying material had crossed
a fractal scale boundary from $S{=}+1$ into $S{=}0$. The apparent creation is the
$S{=}0$ signature of $S{=}+1$ material arriving through the Möbius boundary, exactly
as a star falling into a galactic core at $S{=}0$ becomes a photon traveling at $c$
at $S{=}+1$.

\textbf{The He$+$laser laboratory test.} A direct experimental test follows from this
reasoning. A confined ultra-pure helium-4 cell illuminated with high-intensity photons
from heavy-element laser lines (providing the dense $S{=}{-1}$ analogue of galactic
jet input) is the $S{=}{-1}$ equivalent of the cross-void pumping mechanism. At
intensity and wavelength thresholds corresponding to $\lambda_\text{local} \sim 10$--$30$
(from atomic radii vs.\ focal spot geometry), the FSEP predicts the onset of
hydrogen Balmer emission lines (H$\alpha$ at 656.28~nm, H$\beta$ at 486.13~nm) in
the cell output, accompanied by a measurable anomalous pressure rise $\Delta P$ at
constant volume and temperature that cannot be accounted for by thermal effects alone.

The pressure rise $\Delta P = \Delta n \cdot kT/V$ provides a direct baryon-counting
diagnostic: $\Delta n > 0$ means new atoms have appeared, and their number is
measurable thermodynamically independent of spectroscopic identification. The Balmer
line signature and the pressure anomaly together constitute a two-channel test. If
null across all intensities and wavelengths accessible to the apparatus, the result
constrains the energy threshold for $S{=}{-1} \to S{=}0$ boundary crossing in the
FSEP framework.

This is the most speculative element of the FSEP framework as currently developed,
and the most directly falsifiable. It is included here as a structural prediction
demanded by the fractal analogy, with the explicit acknowledgment that quantitative
derivation of the threshold from Apollonian geometry---analogous to deriving the
work function from $\lambda_\text{local}$---remains future work.
...
sphere packing. Taking this fractal structure as \textit{fundamental} rather than emergent,
the explicit boundary-crossing rules of Section~\ref{sec:core_equations} produce universal
bipolar jets and quantum/galactic non-locality from a single mechanism.
...
manifold physics by the fractal substrate that lies beneath it. Cross-void quasar
correlations appearing superluminal from $S{=}0$ provide the first rung of a true
cosmic distance ladder rooted in the causal geometry of the substrate, bypassing the
redshift-distance conflation that generates the Hubble tension. Bell correlations and
large-scale void spin alignments are the same cross-scale causal mechanism viewed at
different positions on the scale ladder. The CMB is not a relic of a hot early
universe but the resonant eigenmode emission of the fractal hydrogen substrate,
arriving omnidirectionally from the micro-stellar plasma that fills every void at
every scale below $S{=}0$.
...
A. Einstein,
Ann. Phys. (Leipzig) 49, 769 (1916).
\bibitem{Penrose1965}
R. Penrose,
Phys. Rev. Lett. 14, 57 (1965).
\bibitem{Hawking1970}
S. W. Hawking,
Phys. Rev. Lett. 26, 1344 (1970).
\bibitem{HawkingEllis1973}
S. W. Hawking and G. F. R. Ellis,
\emph{The Large Scale Structure of Space-Time} (Cambridge University Press, Cambridge, 1973).
...
\bibitem{Jiao2023}
Y. Jiao \textit{et al.},
Astron. Astrophys. 678, A208 (2023).
\bibitem{Nikonov2023}
A. S. Nikonov \textit{et al.},
Mon. Not. R. Astron. Soc. 526, 5949 (2023).
\bibitem{Planck2014}
Planck Collaboration,
Astron. Astrophys. 571, A27 (2014).
\bibitem{Lineweaver1996}
C. H. Lineweaver \textit{et al.},
Astrophys. J. 470, 38 (1996).
\bibitem{Huang2024}
H.-L. Huang \textit{et al.},
Phys. Rev. D 110, 103540 (2024).

...
The metric form
\[
  ds^2=-c^2d\tau^2+d\ell_G^2
\]
is consistent with the TFOFT definition of time as a Gyro-Ball clock register.
Kepler closure supplies a quadratic period relation,
\[
  T_{\rm orb}^2=\frac{4\pi^2}{GM}a^3,
\]
so the continuum time coordinate naturally appears through a squared proper-time
element. In CLASSP this becomes
\[
  d\tau=e^u dt,\qquad
  u=\ln(R/R_0),
\]
and hence
\[
  ds^2=-c^2e^{2u}dt^2+d\ell_G^2.
\]
Thus the Lorentzian time term is the coarse-grained form of a squared
Gyro-Ball orbital clock, rather than an independently postulated continuum
time dimension.
...
The microscopic CLASSP interaction is path-local: an Info Ball propagates along
an actual active tangency channel.  The continuum Dirac equation is the
long-wavelength limit of that channel update.  Consequently, the ultraviolet
problem of continuum QFT is not present at the microscopic level; there is no
completed continuum of arbitrarily short virtual modes to integrate over.
When continuum observers use Feynman diagrams, the required renormalized
parameters encode the finite ASSP cutoff, hidden lower-layer channels, and
unresolved tangency-path structure.
...
is sharper: when using the correct scale, the TFOFT
...
The immediate empirical test is galactic. Using the Fermi-bubble scale
of $50\,\kly\approx15\,\kpc$ put the predicted equatorial

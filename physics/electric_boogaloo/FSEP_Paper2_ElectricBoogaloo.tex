...
Real motion encounters 3 spatial dimensions + 1 temporal dimension, but FSEP identifies the temporal component as fractal scale latency:
\begin{equation}
Z_\text{naive} = 4\pi^3 + \pi^2 + (3_\text{space} + 1_\text{FSF}) = 136.8947 \quad (0.103\%\,\text{error}),
\end{equation}
where $1_\text{FSF} \approx 10^{-33}$ is the naive temporal drag. The spatial component remains $3$, but FSEP refinement replaces this with $\pi \approx 3.142$ (Apollonian curvilinear propagation), yielding the difference $3.142 - 3 = 0.142$ needed for exact match.
\subsection{Fractal Scale Factor (FSF) Correction}
FSEP replaces (3_\text{space} + 1_\text{FSF}) with curvilinear + cross-scale drag:
\begin{equation}
